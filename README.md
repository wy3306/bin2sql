...
### 🌟 核心升级
1.  **历史延迟精确回溯**：
    *   **痛点解决**：彻底解决了分析历史 Binlog（如几天前的日志）时，计算出的“主从延迟”显示为巨大数值（日志年龄）的问题。
    *   **技术原理**：通过深度解析 GTID 事件中的 `OriginalCommitTimestamp`（主库提交时间）和 `ImmediateCommitTimestamp`（从库落盘时间），还原了事务发生**当时**的真实主从延迟，而非分析时的延迟。
2.  **大事务透视增强**：
    *   新增了**操作类型细分**：不仅统计影响行数，还能具体识别大事务中包含了哪些操作（如 `INSERT`, `UPDATE`, `DELETE`, `DDL`）。
    *   **排序优化**：大事务报告默认改为按**事务开始时间**（StartTime）升序排列，还原事务发生的真实时间线。
    *   **微秒级精度**：事务时间统计全面升级为微秒级（.000000），精确捕捉高频交易场景下的时序细节。
    *   优化了事务时间统计：精确对齐 GTID/BEGIN/COMMIT 事件，解决了长事务时间统计偏差的问题。

### ✅ 修复与优化
1.  **智能网络适应**：优化了智能 Checksum 策略，通过启动时的 **TCP Ping** 测速来动态决定是否开启校验。
    *   延迟 < 10ms：自动关闭 Checksum（极速模式）。
    *   延迟 ≥ 10ms：自动开启 Checksum（安全模式）。
2.  **统计校准**：修复了 DML 统计数与大事务影响行数在某些场景下不一致的问题。
3.  **兼容性**：修正了命令行参数中存在的字符兼容性问题。
