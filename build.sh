...
CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o bin/binlog-analyzer-linux-amd64 main.go
echo "✅ Linux AMD64 编译成功: bin/binlog-analyzer-linux-amd64"

# 2. 编译 Linux ARM64 版本
echo "正在编译 Linux ARM64 版本..."
CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -o bin/binlog-analyzer-linux-arm64 main.go
echo "✅ Linux ARM64 编译成功: bin/binlog-analyzer-linux-arm64"

# 3. 编译 macOS ARM64 版本 (本地测试用)
echo "正在编译 macOS ARM64 版本 (方便本地测试)..."
CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -o bin/binlog-analyzer-darwin-arm64 main.go
